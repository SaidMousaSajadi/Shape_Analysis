...
% imshow(~bwsk == bw , 'Parent',Ax9)

bwth = bwmorph(~bw,'thin',Inf) ;
% imshow(~bwth == bw , 'Parent',Ax9)
...
for k = 1:length(XC)
    ...
    RrMIC(k) = rMIC(MIC_index) ;
    EeMIC(k) = eMIC(MIC_index) ;
    XxMIC(k) = xMIC(MIC_index) ;
    YyMIC(k) = yMIC(MIC_index) ;
end % Main Loop
w1 = 1 ; w2 = 3 ;
